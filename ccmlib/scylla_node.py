            ...
            while time.time() < endtime:
                ...
                if tofind.issubset(live):
                    ...
                    if not have_no_tokens:
                        ...
                        found = set([addr for addr, id in host_id_map.items() \
                                    if addr in tofind_host_id_map and \
                                    (id == tofind_host_id_map[addr] or not tofind_host_id_map[addr])])
                        if found == tofind:
                            return
                time.sleep(0.1)
            self.debug(f"watch_rest_for_alive: found={found} tofind={tofind}: host_id_map={host_id_map} tofind_host_id_map={tofind_host_id_map}")
